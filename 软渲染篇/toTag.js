...
class TGAImage {
  ...
  _loadURL(url) {
    this._src = url;
    this._buffer = readFileSync(url);
    this._parseData();
  }

  _requestBinaryFile(url) {
    return new Promise((resolve, reject) => {
      resolve(readFileSync(url));
    });
  }
  ...
  _initImage() {
    ...
    this._imageData = {
      data: new Array(this._imageWidth * this._imageHeight * 4).fill(0),
    };
  }
  ...
  _getImageData() {
    ...
    if (this._imageType !== _ImageType.none) {
      const colorMapDataLen =
        Math.ceil(this._colorMapDepth / 8) * this._colorMapLength;
      const start = _headerLength + this._idLength + colorMapDataLen;
      data = this._buffer.subarray(start);
    }

    if (
      this._imageType === _ImageType.runlengthColorMapped ||
...
fileList.forEach((name) => {
  if (/^african_head[a-zA-Z_]+\.tga$/.test(name)) {
    const filename =
      'tga' +
      name
        .slice(13, name.length - 4)
        .split('_')
    ...
    tga.onload = () => {
      writeFileSync(
        filename + '.js',
        'const ' + filename + ' = ' + JSON.stringify(tga.imageData.data)
      );
    };
    tga.src = './' + name;
  }
