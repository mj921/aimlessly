...
BaseLine.prototype.renderCanvas = function () {
  const splitNum = 5;
  ...
  this.data.list.forEach(({ label, data }, i) => {
    ...
    data.forEach(({ value }, j) => {
      if (i > 0) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          left + ((i - 1) * (data.length + 1) + (j + 1.5)) * size,
          top +
            (height - top - bottom) *
              (1 - this.data.list[i - 1].data[j].value / max)
        );
        ctx.lineTo(
          left + (i * (data.length + 1) + (j + 1.5)) * size,
...
BaseLine.prototype.renderSvg = function () {
  const splitNum = 5;
  ...
  this.data.list.forEach(({ label, data }, i) => {
    ...
    data.forEach(({ value }, j) => {
      if (i > 0) {
        ...
        line.setAttribute('style', `stroke: ${colors[j]}`);
        svg.appendChild(line);
      }
      ...
      circle.setAttribute('style', `fill: ${colors[j]}`);
...
BaseLine.prototype.update = function (data) {
  this.data = data;
  this.render();
};
